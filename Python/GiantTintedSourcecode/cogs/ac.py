...
class ac(Cog):
	...
	async def checkUser(self, entry, guild):
		...
		if len(whitelist.fetchall()) > 0 or entry.user.id == guild.owner_id or entry.user.id == self.bot.user.id or entry.user.id == guild.role.id:
			return True
	...
	@commands.has_permissions(administrator = True)
	async def whitelist(self, ctx, target: Union[disnake.Member, disnake.Role] = None):
		...
		if target and ctx.author.top_role.position > ctx.guild.me.top_role.position or ctx.author == ctx.guild.owner:
			...
			if isinstance(target, disnake.Role):
				roles = target if type(target) ==disnake.Role else disnake.Object(target)
				view = disnake.ui.View()
				view.add_item(disnake.ui.Button(label = "Добавить", emoji = "♻", custom_id = "add", style = disnake.ButtonStyle.green))
				view.add_item(disnake.ui.Button(label = "Удалить", emoji = "⛔", custom_id = "remove",style = disnake.ButtonStyle.danger))
				view.add_item(disnake.ui.Button(label = "Отмена", emoji = "❌", custom_id = "cancel",style = disnake.ButtonStyle.blurple))
				message = await ctx.send(
					embed = disnake.Embed(
						title = "Подтверждение",
						description=f">>> **Вы уверены, что хотите добавить в вайтлист {roles.name}? Ведь после этого, бот не будет обращать внимание на его действия на сервере!**",
						color = disnake.Colour(self.color)
					),view = view
				)
				inter = await self.bot.wait_for('Button_click', check = lambda i: i.author == ctx.author)
				if inter.component.custom_id == 'add':
					await message.edit(embed = disnake.Embed(title = ':gear: | Вайтлист',
						description = f">>> **Теперь все действия пользователя у которого есть роль `{roles.name}` игнорируются.**",
						color = disnake.Colour.green()
					),view = None)
					self.cursor.execute(f"DELETE FROM wl WHERE id = {roles.id} AND guild = {ctx.guild.id}")
					self.cursor.execute("INSERT INTO wl VALUES(?, ?)", (ctx.guild.id, roles.id))
					self.data.commit()

				elif inter.component.custom_id == 'remove':
					await message.edit(embed = disnake.Embed(title = ':gear: | Вайтлист',
						description = f">>> **Вы убрали `{roles.name}` с вайтлиста**",
						color = disnake.Colour(self.color)
					),view = None)
					self.cursor.execute(f"DELETE FROM wl WHERE id = {roles.id} AND guild = {ctx.guild.id}")
					self.data.commit()
