...
Бустов: `{ctx.guild.premium_subscription_count}`**""",
	...
	async def mute(self, ctx, member: disnake.Member = None, reason=None):
		...
		if member:
			...
			message = await ctx.send(embed=disnake.Embed(description=f">>> **Насколько мне замутить пользователя: {member.name}**",color = disnake.Colour(self.color)), view = view)
			inter = await self.bot.wait_for('button_click', check=lambda i: i.author == ctx.author)
			...
			await ctx.send(embed=disnake.Embed(title = '✅ | Мут',
